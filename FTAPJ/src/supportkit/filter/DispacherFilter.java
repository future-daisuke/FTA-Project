...
public class DispacherFilter implements Filter {
	...
	public void doFilter(ServletRequest servletReq, ServletResponse servletRes, FilterChain chain) throws IOException, ServletException {
		...
		for (File file : searcher.listFilesWithPattern(contextPath, ".*\\.jsp")) {
			String resourcePath = file.getAbsolutePath().substring(contextPath.length()).replace("\\", "/");
			resoruceList.add(resourcePath);
		}

		// 存在するリソースを要求しているか判定
		// ディスパッチ先を「./」のようにウェルカムページを指定しているケースががあるが、
		// dispacherRequestPathには正しいindex.jspなどが格納される
		if (!CollectionUtils.contais(dispatcherRequestPath, resoruceList)) {
